...
function CardBenefitsOverview({ quarter }) {
  return (
    <div className="p-4 space-y-3">
      <div className="text-center mb-2">
        <p className="text-xs text-gray-400">Navigate to a merchant site for personalized recommendations</p>
      </div>
      ...
      {cards.map((card) => {
        const fixedEntries = Object.entries(card.fixedBenefits).filter(
          ([key]) => key !== 'other'
        );
            ...
            {fixedEntries.length > 0 && (
              <div className="mb-2">
                ...
                <div className="space-y-1">
                  {fixedEntries.map(([key, benefit]) => (
                    <div key={key} className="flex items-center justify-between">
                      <span className="text-[11px] text-gray-600 capitalize">{formatCategory(key)}</span>
                      <span
                        className="text-[11px] font-bold"
                        style={{ color: card.color }}
                      >
                        {benefit.rate}{benefit.unit}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="mb-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-gray-600">
                    {rotation.categories.map(formatCategory).join(', ')}
                  </span>
                  <span className="text-[11px] font-bold text-green-600">
                    {rotation.rate}{rotation.unit === 'x' ? 'x' : '%'}
                  </span>
                </div>
                <p className="text-[10px] text-gray-400 mt-0.5">
                  Up to ${rotation.limit.toLocaleString()}
                </p>
              </div>
            )}
...
function formatCategory(key) {
  const names = {
    dining: 'Dining',
    streaming: 'Streaming',
    onlineGrocery: 'Online Grocery',
    travel: 'Chase Travel',
    generalTravel: 'Other Travel',
...
export default function SmartMatch({ category, merchantName, quarter }) {
  ...
  if (!category) {
    return <CardBenefitsOverview quarter={quarter} />;
  }
